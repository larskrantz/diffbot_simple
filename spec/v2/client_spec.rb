...
module DiffbotSimple::V2
	describe Client do
		let(:fake_bulk_factory) { double("bulk_factory") }
		let(:subject) { Client.new token: "FOO_TOKEN", bulk_factory: fake_bulk_factory }
		...
		context "when asking for bulk, "
			context "all" do
				...
				it "should use bulk factory" do
			 		expect(fake_bulk_factory).to receive(:all)
			 		all_bulk
				end
			end
			context "named" do
				...
				it "should use bulk factory" do
			 		expect(fake_bulk_factory).to receive(:single).with({ name: :foo })
			 		named
				end
		end
	end
